...
router.patch('/patient-update/:id',async(req,res)=>{
    try {
        ...
        if (!useremail && !usermobilenumber) {
            ...
            const patient=await Patient.findByIdAndUpdate(req.patient._id,{...req.body},{new:true});
...
router.delete("/patient-remove/:id" , async function(req, res) {
    try {
        ...
        res.status(200).send({message:"Patient Deleted Successfully",data:patient});
    } catch (error) {
        res.status(500).send({message:"Internal Server Error"});
        console.log(error);
    }
...
router.post("password-reset/:id/:token", async (req, res) => {
	try {
        ...
        const patient=await new Patient({...req.body,password:hashPassword,confirmpassword:hashconfirmpassword}).save();
		await token.remove();
...
router.post('/OPDBooking', async (req, res) => {
    ...
    await sendEmail(email, `OPD BOOkED on ${details.appointmentDate} at ${details.appointmentTime}`, url);
